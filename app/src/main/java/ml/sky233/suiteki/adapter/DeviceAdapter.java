...
public class DeviceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements OnItemClickListener, OnItemLongClickListener, OnItemTouchListener {
    public Context mContext;

    private List<DeviceInfo> mPublicArray;
    private DevicesList devicesList;

    public DeviceAdapter(Context context, DevicesList publicArray) {
        mContext = context;
        devicesList = publicArray;
        mPublicArray = publicArray.getList();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup vg, int viewType) {
        ...
        View v = LayoutInflater.from(mContext).inflate(R.layout.list_item_device, vg, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder vh, final int position) {
        ...
        holder.tv_title.setText(mPublicArray.get(position).Name);
        holder.tv_mac.setText(mPublicArray.get(position).Mac);
        if(position == devicesList.getDeviceInfoIndex())
            holder.iv_line.setVisibility(View.VISIBLE);
//        holder.iv_img.setImageResource(mPublicArray.get(position));
        holder.iv_img.setImageResource(R.drawable.ic_device_miband_7);
    ...
    public static class ItemHolder extends RecyclerView.ViewHolder {
        ...
        public TextView tv_mac; // 声明列表项标题的文本视图
        ...
        public ItemHolder(View v) {
            super(v);
            tv_title = v.findViewById(R.id.item_device_title);
            iv_line = v.findViewById(R.id.item_img);
            iv_img = v.findViewById(R.id.item_watchface_img);
            tv_mac = v.findViewById(R.id.item_device_mac);
        }
    }
...
//        Toast.makeText(mContext, desc, Toast.LENGTH_SHORT).show();
    ...
    public void onItemLongClick(View view, int position) {
        String desc = String.format("您长按了第%d项，标题是%s", position + 1,
                mPublicArray.get(position).Name);
        Toast.makeText(mContext, desc, Toast.LENGTH_SHORT).show();
    }
