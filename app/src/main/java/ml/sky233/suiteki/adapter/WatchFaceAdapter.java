...
public class WatchFaceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements OnItemClickListener, OnItemLongClickListener, OnItemTouchListener {
    public Context mContext;
    public String title;

    private ArrayList<WatchFaceObject> mPublicArray;

    public WatchFaceAdapter(Context context, ArrayList<WatchFaceObject> publicArray) {
        mContext = context;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup vg, int viewType) {
        ...
        View v = LayoutInflater.from(mContext).inflate(R.layout.list_item_watchface, vg, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder vh, final int position) {
        ...
        if (!mPublicArray.get(position).user.equals("")) {
            holder.tv_user.setText(mPublicArray.get(position).user);
        }
        if (!mPublicArray.get(position).img.equals(""))
            holder.iv_img.setImageFromUrl(mPublicArray.get(position).img);
        Log.d(MainApplication.TAG,mPublicArray.get(position).img);
    ...
    public static class ItemHolder extends RecyclerView.ViewHolder {
        ...
        public TextView tv_user; // 声明列表项标题的文本视图
        public SuitekiCardButton btn;

        public SuitekiImageView iv_img; // 声明列表项描述的文本视图

        public ItemHolder(View v) {
            super(v);
            tv_title = v.findViewById(R.id.textView_title);
            tv_user = v.findViewById(R.id.textView_user);
            btn = v.findViewById(R.id.watch_btn);
